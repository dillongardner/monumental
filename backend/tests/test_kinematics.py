...
def test_simple_cases():
    ...
    swing_lift_elbow = CraneService.xyz_to_swing_lift_elbow(
        xyz_for_zeros, DEFAULT_CRANE
    )
...
def test_orientation_change():
    ...
    swe_no_orientation = CraneService.xyz_to_swing_lift_elbow(
        xyz=xyz, crane=DEFAULT_CRANE
    )
    assert swe_no_orientation is None
    swe_orientation = CraneService.xyz_to_swing_lift_elbow(
        xyz=xyz, crane=DEFAULT_CRANE, orientation=orientation
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            CraneOrientation(x=0, y=0, z=0, rotationZ=0),
        ),
        (
            SwingLiftElbow(swing=0, lift=0, elbow=0),
            CraneOrientation(x=0, y=0, z=0, rotationZ=90),
        ),
        (
            SwingLiftElbow(swing=45, lift=1, elbow=90),
            CraneOrientation(x=0, y=0, z=0, rotationZ=180),
        ),
        (
            SwingLiftElbow(swing=-30, lift=2.5, elbow=45),
            CraneOrientation(x=0.4, y=0.3, z=0.7, rotationZ=-90),
        ),
        (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            XYZPosition(x=-0.5, y=-1, z=0.5),
            CraneOrientation(x=0.3, y=0.5, z=1, rotationZ=10),
        ),
